...
export default function Cards({ data }: CardsProps) {
  ...
  return (
    <div className="grid grid-rows-3 gap-3 lg:grid-cols-3 lg:grid-rows-1 lg:gap-[14px] my-10">
      {data.map((item, index) =>
        isDesktop ? (
          <CardDesktop key={index} {...item} varient={varients[index % 3]} />
        ) : (
...
export function CardDesktop({
  ...
  return (
    ...
    >
      ...
      <div
        className={`h-[400px] w-full p-12 rounded-lg absolute z-10 bg-4-6 ${
          open && "-translate-y-[400px]"
